...
def get_pocket_ligand(
    ...
    try:
        ...
        print("The protein", pdb_id, "has NO LIGAND inside the defined pocket.")
    ...
    lig_sel = lig.select('(within ' + str(cutoff) + ' of prot) and not water', prot = protein_pocket)
    if lig_sel is None:
    ...
    print(lig_resnums)
    print(lig_sel.getResnames())
    lig_names = np.unique( lig_sel.getResnames() )
...
#                     #print(chain, resnum, resname, dist_new_mol)
    ...
    num_heavy_atoms = true_lig.select('not name "H.*"').getNames().size
    ...
    if true_lig != "" and num_heavy_atoms > min_heavy_atoms: # Si el ligando existe y su masa fue mayor 96
        ...
        if write_files:
            ...
            print(f'Protein {pdb_id}:\n   Molecules found: {str(lig_names)} ' + 
                  f'-> ligand {name_lig} saved ({dist_new_mol:.2f} A from pkt center)' )
            writePDB( f"{pk_ligs_dir}/{pdb_id}_{name_lig}_LIG.pdb", 
                      complejo_PL.select(f"resname {name_lig}") )
        return (name_lig, lig_mw, num_heavy_atoms)
    else: 
