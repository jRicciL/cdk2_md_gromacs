...
def cMDS_projection(cMDS_obj: Tuple, 
    ...
    Modified from: http://www.nervouscomputer.com/hfs/cmdscale-in-python/
    '''
    # Get matrices from the cMDS object 
    F = cMDS_obj[0].T # F matrix
    D_sq = cMDS_obj[1] # Delta^2 matrix
    e_vals = cMDS_obj[3] # Eigenvalues 
    n = len(D_sq)
    e1 = np.ones((n, 1))
    m = len(sup_point)
    e1_sup = np.ones((m, 1))
    m = ( np.ones((n, 1)) / n)
    I = np.eye(n) # I_n, identity matrix
